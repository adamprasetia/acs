...
class Mod_sche_move extends MY_Controller {
	function __construct(){
		parent::__construct();
		$this->load->helper('mod_helper');
	}
	function index(){		
		$data['title'] = 'ACS - Moderation Schedule MOVE';
		//--HARI INI--//
		$this->table->set_template(tbl_tmp());
		$this->table->set_heading(array('data'=>'SHIFT','width'=>20),'MODERATOR');
		$shift_siang = get_moderator(siang_move(date('Y-m-d')));
		$shift_malam = get_moderator(malam_move(date('Y-m-d')));
		...
		for($i=1;$i<=$total_hari;$i++){
			...
			$shift = get_moderator(siang_move($date));
			$row[] = array('data'=>$shift[0],'class'=>'mod_'.siang_move($date));
		}
		...
		for($i=1;$i<=$total_hari;$i++){
			$date = date_format(date_create(date('Y-m-',strtotime('last day of last month')).$i),"Y-m-d");
			$shift = get_moderator(malam_move($date));
			$row[] = array('data'=>$shift[0],'class'=>'mod_'.malam_move($date));
		}
		$this->table->add_row($row);
		unset($row);
