...
class Barang_masuk extends MY_Controller {
	...
	function index(){
		...
		$data['title'] = 'ACS - Barang Masuk';
		$data['action'] = site_url('barang_masuk/search'.$this->_filter());
		$data['action_add'] = site_url('barang_masuk/add'.$this->_filter());
		$data['export_btn'] = anchor('barang_masuk/export'.$this->_filter(),'<span class="glyphicon glyphicon-export"></span> Export Excel 2007',array('class'=>'btn btn-primary btn-sm'));
		...
		foreach($head_data as $r => $value){
			$heading[] = anchor('barang_masuk'.$this->_filter(array('order_column'=>$r,'order_type'=>$this->lib_general->order_type($r))),$value." ".$this->lib_general->order_icon($r));
		}		
		$heading[] = 'Action';
		$this->table->set_heading($heading);
		$result = $this->mdl_barang_masuk->get()->result();
		$i=1+$offset;
		foreach($result as $r){
			$this->table->add_row(
				$i++
				,anchor('barang_masuk/edit/'.$r->id.$this->_filter(),$r->barang_name)
				,format_tanggal($r->tanggal)	
				,$r->banyak	
				,$r->vendor_name	
				,$r->operator	
				,$r->keterangan
				,anchor('barang_masuk/edit/'.$r->id.$this->_filter(),'<span class="glyphicon glyphicon-edit"></span> Edit')
				."&nbsp;|&nbsp;".anchor('barang_masuk/delete/'.$r->id.$this->_filter(),'<span class="glyphicon glyphicon-trash"></span> Delete',array('onclick'=>"return confirm('Are you sure')"))
			);
		}
		$data['table'] = $this->table->generate();
		$total = $this->mdl_barang_masuk->count_all();
		
		$config = pag_tmp();
		$config['base_url'] = site_url("barang_masuk".$this->_filter());
		...
		$this->template->display('barang_masuk',$data);
	}
	function _set_rules(){
	...
	function add(){
		...
		if($this->form_validation->run()===false){
			$data['title'] = 'ACS - New';
			$data['breadcrumb'] = 'barang_masuk'.$this->_filter();
			$data['heading'] = 'New';
			$data['owner'] = '';
			$data['error'] = validation_errors();
			$data['action'] = form_open('barang_masuk/add'.$this->_filter());
			$this->template->display('barang_masuk_edit',$data);
		}else{			
			$data = $this->_field();
			$data['user_create'] = $this->session->userdata('user_login');
			$data['date_create'] = date('Y-m-d H:i:s');		
			$this->mdl_barang_masuk->add($data);
			$this->session->set_flashdata('alert','<div class="alert alert-success">Complete...!!!</div>');
			redirect('barang_masuk'.$this->_filter());
		}
	}
	function edit($id){
		$this->_set_rules();
		if($this->form_validation->run()===false){			
			...
			$data['heading'] = 'Update';
			$data['error'] = validation_errors();
			$data['action'] = form_open('barang_masuk/edit/'.$id.$this->_filter());
			$data['close'] = form_close();
			$data['row'] = $this->mdl_barang_masuk->get_from_field('id',$id)->row();
			...
			$this->mdl_barang_masuk->edit($id,$data);
			$this->session->set_flashdata('alert','<div class="alert alert-success">Complete...!!!</div>');
			redirect('barang_masuk/edit/'.$id.$this->_filter());
		}
	}
	function delete($id){		
		$this->mdl_barang_masuk->delete($id);
		$this->session->set_flashdata('alert','<div class="alert alert-success">Complete...!!!</div>');
		redirect('barang_masuk'.$this->_filter());
	}
	function search(){
		...
		redirect('barang_masuk'.$this->_filter($data));
	}
	...
	function export(){
		...
		$active_sheet->setTitle('Barang Masuk List');
		...
		$result = $this->mdl_barang_masuk->export($order_column,$order_type)->result();
		$i=2;
		foreach($result as $r){
		...
		$filename='LIST_BARANG_MASUK_'.date('Ymd_His').'.xlsx';
